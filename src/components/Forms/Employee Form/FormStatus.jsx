...
const FormStatus = ({formState}) => {
  ...
  return (
    <div className={`w-full  h-full  flex px-3 max-md:px-10 justify-center items-center `}>
      ...
      <p className='absolute mt-11 max-sm:text-xs'>Primary</p>
      </div>
     ...
     <p className=' absolute mt-11 w-36 translate-x-4 max-sm:text-xs max-sm:translate-x-8'>Personal Details</p>
     </div>
     ...
     <div className={`w-40 border border-dashed ${formState>2?"border-yellow-600": Theme=="Dark" ? "border-slate-200"  : "border-black"}  h-0`}></div>
      ...
      <p className=' absolute mt-11   w-36 translate-x-4 max-sm:text-xs max-sm:translate-x-8'>Additional Details</p>
      </div>
    </div>
  )
}
